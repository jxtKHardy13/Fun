...
WEBHOOK_URL = "https://your-ngrok-id.ngrok-free.app:8443/webhook"  # Replace with your ngrok URL or production domain
...
# =============================================================================
def init_db():
    """Initialize SQLite database for wallets, trades, and settings."""
...
async def generate_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Generate a new Solana wallet and store it."""
    ...
    try:
        ...
        message = (
            ...
            f"Store your mnemonic and private key offline. Losing them means losing access to your wallet."
        )
...
async def execute_trade(user_id: int, token_address: str, amount: float, action: str = "buy") -> bool:
    """Execute a trade on a DEX (placeholder with slippage)."""
    async with wallet_lock:
...
async def wallet_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "🔒 For maximum security, use /uploadkey to upload a file."
    )
    async with orders_lock:
        pending_wallet[user_id] = True

async def uploadkey(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Prompt user to upload a key file."""
    user_id = update.effective_user.id
    await update.message.reply_text(
        "📤 Upload a text file containing your mnemonic (12/24 words) or private key (Base58, hex, or JSON array).\n"
...
async def help_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_message = (
        "📚 Help Center\n\n"
        "**How to Use This Bot**\n"
        "1. **Connect Wallet**: Use /wallet or 'Connect Wallet' button to connect an existing wallet.\n"
        "2. **Generate Wallet**: Use 'Generate Wallet' button to create a new Solana wallet.\n"
        "3. **Start Trading**: Use /buysell to trade tokens. Example: `/buysell TOKEN_ADDRESS, 1.0`\n"
        "4. **Snipe Tokens**: Use /sniper to enable Pump.fun sniper mode.\n"
        "5. **Set Limit Orders**: Use /limitorders to create/modify limit orders.\n"
        "6. **Set Up DCA**: Use /createdca <TOKEN> <AMOUNT> <INTERVAL>. Example: `/createdca SOL 0.1 3600`\n"
        "7. **Copy Trade**: Use /copytrade to follow a trader's address.\n"
        "8. **View Portfolio**: Use /profile to see wallet balance and trade stats.\n"
        "9. **Adjust Settings**: Use /settings to toggle autobuy, autosell, or set slippage.\n"
        "10. **Backup Settings**: Use /exportsettings to save your settings.\n\n"
        "Happy trading! 💊"
    )
...
async def pending_input_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if order["action"] == "trade":
        ...
        await update.message.reply_text("❌ File upload not implemented yet. Please paste the key directly.")
        async with orders_lock:
            if user_id in pending_orders:
                del pending_orders[user_id]
...
# =============================================================================
def main() -> None:
    """Start the bot with webhook for 24/7 operation."""
    global application
...
if __name__ == "__main__":
    main()    key_bytes = fernet.decrypt(encrypted_key)
    return Keypair.from_bytes(key_bytes)
